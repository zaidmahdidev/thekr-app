...
class _QuranScreenState extends ConsumerState<QuranScreen> with WidgetsBindingObserver {
  ...
  Future<void> _shareCurrentPage() async {
    ...
    try {
      ...
      if (uint8list != null) {
        ...
        final imagePath = File('${directory.path}/quran_page_${state.currentPage}.png');
        await imagePath.writeAsBytes(uint8list);

        await Share.shareXFiles(
          [XFile(imagePath.path)],
          text: 'من تطبيق ${AppConstants.appName}\n${AppConstants.playStoreUrl}',
        );
      }
    } catch (e) {
      notifier.setCapturing(false);
      ...
      child: QuranLibraryScreen(
        ...
        topBarStyle: QuranTopBarStyle.defaults(
          ...
          customTopBarWidgets: [
            IconButton(
              onPressed: () {
                ref.read(quranProvider(widget.currentPage).notifier).toggleDarkMode();
              },
              icon: Icon(
        ...
        onPageChanged: (page) {
          ref.read(quranProvider(widget.currentPage).notifier).updatePage(page + 1);
        },
        backgroundColor: isDarkMode 
            ? const Color(0xFF121212) 
            : const Color(0xFFFFFDF5),
        textColor: isDarkMode ? Colors.white.withValues(alpha: 0.9) : Colors.black87,
        ayahIconColor: context.colors.primary,
        ayahSelectedBackgroundColor: context.colors.primary.withValues(alpha: 0.15),
      ),
    );
  }
}
